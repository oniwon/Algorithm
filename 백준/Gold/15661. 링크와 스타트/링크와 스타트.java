...
public class Main {
    ...
    public static void dfs(int depth, int count, int teamSize) {
        if(count == teamSize) { // 재귀 탈출 조건: 지정된 팀원 수가 채워질 때
            calculateDifference();
            return;
        }
        ...
        for(int i = depth; i < n; i++) {
            ...
            dfs(i + 1, count + 1, teamSize); // 다음 선수 선택
            visited[i] = false;
        }
    }
